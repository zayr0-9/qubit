...
func newMdDocumentComposer() composerModel {
	c := newComposer()
	c.placeholder = "write markdown..."
	c.maxHeight = 1000
	c.maxContentHeight = 4000
	return c
}

func newMdRenameComposer() composerModel {
	c := newComposer()
	c.placeholder = "filename"
	c.maxHeight = 1
	return c
}
...
func (m *model) layoutMdEditorComposer() {
	...
	m.mdEditor.Editor.maxHeight = m.mdEditorVisibleEditorHeight()
...
func (m model) renderMdEditorEdit(height int) string {
	...
	editorModel.maxHeight = editorHeight
