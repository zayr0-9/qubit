...
func toolGroupStatus(g *toolGroup) string {
...
func toolGroupTotalDurationMs(g *toolGroup) int {
	if g == nil {
		return 0
	}
	total := 0
...
func (m *model) renderToolGroupInline(group *toolGroup) string {
	...
	if showDevToolDetails() {
		if duration := toolGroupTotalDurationMs(group); duration > 0 {
...
func (m *model) hasRunningToolGroup() bool {
	for i := range m.messages {
		group := m.messages[i].ToolGroup
		if group != nil && toolGroupStatus(group) == "running" {
			return true
		}
	}
	return false
