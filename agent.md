...
- Plan/edit mode maps the UI's permission mode to runtime prompt mode: plan uses ask-before-gated-tools behavior and the `prompts/plan.md` system prompt addendum; edit uses always-allow gated-tool behavior and the `prompts/edit.md` system prompt addendum. Subagent runs use `prompts/subagent.md` inside hidden child sessions. Keep the Markdown files as the editable source for these prompt addenda. In plan mode, `planMd` and `subagent` are auto-allowed exception tools; `planMd` can use `action: "clarify"` to ask one or more user clarification questions before the final plan; Go renders these in the bottom overlay above the input, always includes a final manual-entry option, and returns all answers to the model as the tool result.
